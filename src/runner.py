        ...
        try:
            ...
            if tag_info.error or not tag_info.has_required_fields:
                if progress_callback:
                    progress_callback(f"Tag missing for {mp3_path.name}, using fallback folder")
            ...
            if progress_callback:
                progress_callback(f"{result.status.upper()}: {mp3_path.name} -> {result.dst.name}")
        except Exception as exc:
            summary["errors"] += 1
            logger.exception(f"Error processing {mp3_path}: {exc}")
            if progress_callback:
                progress_callback(f"ERROR: {mp3_path.name} - {exc}")
